...
class PlexConnector(Connector):
    ...
    def is_configured(self, stored: dict[str, Any] | None) -> bool:
        ...
        return bool(str(resolved.get("plex_token") or "").strip() or str(resolved.get("overseerr_key") or "").strip())
    ...
    async def test_connection(self, config: dict[str, Any]) -> dict[str, Any]:
        ...
        async with httpx.AsyncClient(timeout=timeout) as client:
            if plex_token:
                try:
                    r = await client.get(f"{plex_url}/identity", params={"X-Plex-Token": plex_token})
            ...
            if ovr_key:
                try:
                    r = await client.get(f"{ovr_url}/api/v1/status", headers={"X-Api-Key": ovr_key})
                    if r.status_code == 200:
        ...
        for item in list(root)[:10]:
            ...
            if added:
                with contextlib.suppress(ValueError, TypeError):
                    added_iso = datetime.fromtimestamp(int(added), tz=UTC).isoformat()
            out.append(
                {
                    "title": title,
